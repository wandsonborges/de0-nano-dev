...
architecture bhv of window_gen is
  ...
  type img_mem_type is array(0 to COLS*KERNEL_H-1) of STD_LOGIC_VECTOR(NBITS_DATA-1 downto 0);
  signal img_mem : img_mem_type := (others => (others => '0'));
    ...
    if rst_n = '0' then                 -- asynchronous reset (active low)
      ...
      case wg_state is
        ...
        when st_gettingFirstLine =>
          if (pxl_valid = '1') then
            img_mem(COLS*to_integer(index_firstLine) + to_integer(col_counter)) <= pxl_data;
        ...
        when st_gettingSecondLine =>
          if (pxl_valid = '1') then
            img_mem(COLS*to_integer(index_secondLine) + to_integer(col_counter)) <= pxl_data;
            if (col_counter = COLS-1) then
        ...
        when st_gettingLines =>
          if (pxl_valid = '1') then
            img_mem(COLS*to_integer(index_thirdLine) + to_integer(col_counter)) <= pxl_data;
  ...
  begin  -- process win_proc
    if rst_n = '0' then                 -- asynchronous reset (active low)
      ...
      --if (pxl_valid = '1' and col_counter > KERNEL_W-2 and line_counter > KERNEL_H-2) then
        window_valid <= '1';
        window_data_reg(0)(0) <= img_mem(COLS*to_integer(index_firstLine) + to_integer(col_counter)-2);
        window_data_reg(0)(1) <= (others => '0'); --img_mem(COLS*to_integer(index_firstLine) + to_integer(col_counter)-1);
        window_data_reg(0)(2) <= (others => '0'); --img_mem(COLS*to_integer(index_firstLine) + to_integer(col_counter));

        window_data_reg(1)(0) <= (others => '0'); --img_mem(COLS*to_integer(index_secondLine) + to_integer(col_counter)-2);
        window_data_reg(1)(1) <= (others => '0'); --img_mem(COLS*to_integer(index_secondLine) + to_integer(col_counter)-1);
        window_data_reg(1)(2) <= (others => '0'); --img_mem(COLS*to_integer(index_secondLine) + to_integer(col_counter));

        window_data_reg(2)(0) <= (others => '0'); --img_mem(COLS*to_integer(index_thirdLine) + to_integer(col_counter)-2);
        window_data_reg(2)(1) <= (others => '0'); --img_mem(COLS*to_integer(index_thirdLine) + to_integer(col_counter)-1);
